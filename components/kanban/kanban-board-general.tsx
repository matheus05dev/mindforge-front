...
import type { KanbanColumn as KanbanColumnType, KanbanTask } from "@/lib/api/types"
import type { Task } from "@/lib/types"

export function KanbanBoardGeneral() {
  const { data: columns, loading, error, execute } = useApi<KanbanColumnType[]>()
  ...
  if (loading) {
    return <div className="text-center py-12 text-muted-foreground">Carregando quadro Kanban...</div>
  }

  if (error) {
    const errorMessage = error?.message || "Erro desconhecido ao carregar quadro"
    return (
      <div className="text-center py-12">
        <p className="text-destructive mb-4">Erro ao carregar quadro: {errorMessage}</p>
  ...
  if (!columns || columns.length === 0) {
    return (
      <div className="text-center py-12">
        <p className="text-muted-foreground mb-4">Nenhuma coluna encontrada. Crie uma coluna para começar.</p>
      </div>
    )
  }
        ...
        <div className="flex gap-4 min-w-max h-full">
          {sortedColumns.map((column) => (
            <KanbanColumnApi
              ...
              onEditTask={(task) => {
                const apiTask = column.tasks.find((t) => t.id.toString() === task.id)
                if (apiTask) {
